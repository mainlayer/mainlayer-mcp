...
export const buyerToolDefinitions: Tool[] = [
  {
...
export async function handleBuyerTool(
  name: string,
  ...
  switch (name) {
    case "discover_resources": {
      const resources = await client.discoverResources({
        ...
        fee_model: args.fee_model as string | undefined,
    ...
    case "get_resource_info": {
      const resource = await client.getResourceInfo(args.resource_id as string);
      return resource;
    }
